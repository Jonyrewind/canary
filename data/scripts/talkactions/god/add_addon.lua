...
local addons = TalkAction("/removeaddon")
...
function addons.onSay(player, words, param)
	...
	if param == "" then
		target = player:getTarget()
		if not target then
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Removes players addon for a specific outfit. Usage: /removeaddon <player name>, <looktype>")
			return true
		end
	else
		target = Player(name)
	...
	target:removeOutfitAddon(looktype, addons)
	player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Addon for looktype  " .. looktype .. "a for " .. target:getName() .. " set to " .. addons .. ".")
	return false
end
