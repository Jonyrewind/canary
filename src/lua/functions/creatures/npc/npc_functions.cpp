...
int NpcFunctions::luaNpcSellItem(lua_State* L) {
    ...
    case NpcCurrencyType::ITEM:
        if (!g_game().removeMoney(player, backpackCost, 0, true) ||
            !player->removeItemOfType(npc->getCurrencyItemId(), itemCost, -1, false)) {
    ...
    case NpcCurrencyType::TRANSFERABLE_COINS:
        if (!g_game().removeMoney(player, backpackCost, 0, true) ||
            !player->removeTransferableCoins(itemCost)) {
            g_logger().error("[NpcFunctions::luaNpcSellItem] Player {} lacks Transferable Coins or gold for bags", player->getName());
        }
        break;
	}
