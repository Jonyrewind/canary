...
void Player::applyScrollImbuement(const std::shared_ptr<Item> &item, const std::shared_ptr<Item> &scrollItem) {
	...
	Expand commentComment on line R2493Resolved if (g_game().internalRemoveItem(scrollItem, 1) != RETURNVALUE_NOERROR) {
	...
	Expand commentComment on lines R2494 to R2504Resolved
}
