...
function ferumbrasAscendantTarbazNotes.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if item.actionid == 54388 then
		if player:getStorageValue(Storage.FerumbrasAscension.BasinCounter) ~= 8 or player:getStorageValue(Storage.FerumbrasAscension.TarbazNotes) >= 1 then
			return false
		end
	...
	elseif item.actionid == 54389 then
		if player:getStorageValue(Storage.FerumbrasAscension.BasinCounter) ~= 8 and player:getStorageValue(Storage.FerumbrasAscension.TarbazNotes) < 1 or player:getStorageValue(Storage.FerumbrasAscension.TarbazNotes) >= 2 then
			return false
		end
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, 'Suddenly, you see the letter A. It all makes sense now. And then you now the secret.')
