...
local function creatureSayCallback(npc, creature, type, message)
	...
	elseif MsgContains(message, "yes") and npcHandler:getTopic(playerId) == 2 then
		...
		npcHandler:say(string.format("You have Bought %s for %i gold.", itemList, moneyRequired), npc, player)
		npcHandler:setTopic(playerId, 1)
		npcHandler:say(imbuementPackages, npc, creature)
	elseif MsgContains(message, "no") then
		npcHandler:say("Your decision. Come back if you have changed your mind.", npc, player)
	end
end

npcHandler:setMessage(MESSAGE_GREET, "Hello |PLAYERNAME|, say {imbuement packages} or {trade} to buy imbuement items.")
