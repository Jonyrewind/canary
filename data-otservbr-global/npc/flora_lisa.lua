...
local npcType = Game.createNpcType("Flora Lisa")
local npcConfig = {}
...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "fight") then
		npcHandler:say("Do you really want to enter Lisa's hideout. There is no chickening out and I also have to charge one voting right! {Yes} or {no}?", npc, creature)
		npcHandler:setTopic(playerId, 1)
	elseif MsgContains(message, "yes") then
		if npcHandler:getTopic(playerId) == 1 then
			npcHandler:say("Here you go!", npc, creature)
			local pos = { x = 33558, y = 31905, z = 8 }
